...
export default function TableOfContents({
  ...
  return (
    <aside className="h-auto w-auto">
      <div>
        <div className="font-open-sans mb-4 flex items-center gap-2 p-0 text-[16px] tracking-wide text-[#171717]">
          On this page
        </div>
        <ul className="font-open-sans list-outside space-y-1 text-sm">
          {headings.map((item) => (
            <li key={item.id}>
              <a
                href={`#${item.id}`}
                className={`block px-0 py-1 text-[13px] transition-colors ${
                  activeId === item.id ? 'text-privue-800 font-normal' : 'text-gray-700'
                }`}
              >
                {item.text}
              </a>
