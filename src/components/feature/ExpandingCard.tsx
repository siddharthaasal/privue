...
export default function ExpandingCard({
  ...
  return (
    <motion.article
      ...
      onClick={onClick}
      className={[
        'group/card relative h-64 cursor-pointer overflow-hidden rounded-lg md:h-80',
        'border border-black/10 bg-white shadow-sm',
        isActive ? 'md:col-span-6' : isAnyActive ? 'md:col-span-3' : 'md:col-span-4',
    ...
    >
      <motion.div layout className="absolute inset-0 flex h-full w-full">
        {/* LEFT (Image + Overlay) */}
        <motion.div layoutId={`image-${item.id}`} className="relative w-full overflow-hidden">
          <img src={item.img} alt={item.heading} className="h-full w-full object-cover" />
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent" />
            <motion.div layoutId={`overlay-${item.id}`} className="absolute inset-x-0 bottom-0 p-4">
        ...
        <AnimatePresence mode="sync">
          {isActive && (
            <motion.div
              ...
              className="hidden w-full bg-white/80 backdrop-blur-sm md:flex"
            >
              <div className="flex h-full flex-col justify-between p-6 text-neutral-900">
                <div className="text-xs font-semibold tracking-wide text-neutral-700 uppercase">
                  Highlights
                </div>
