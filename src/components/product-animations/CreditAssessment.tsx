...
function BarRow({ label, score, color }: { label: string; score: number; color?: string }) {
    ...
    return (
        <div className="flex items-center gap-3 w-full">
            ...
            <div className="w-12 text-right text-[11px] font-medium text-slate-800">{Math.round(animated)}</div>
        </div>
    );
}
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-[10px] text-muted-foreground">Live • Compact</div>
            </div>

            <div className="grid grid-cols-2 gap-2 mb-3">
                <StatTile title="Credit Score" value={Math.round(animScore)} sub="Excellent" percent={(animScore / 850) * 100} color="#16a34a" />
                <StatTile title="Payment Score" value={`${Math.round(animPayment)}%`} sub="On-time" percent={animPayment} color="#10b981" />
                <StatTile title="Utilization" value={`${Math.round(animUtil)}%`} sub="Moderate" percent={animUtil} color="#f59e0b" />
                <StatTile title="Default Risk" value={`${animDefault.toFixed(2)}%`} sub="Probability" percent={100 - Math.min(95, animDefault * 20)} color="#059669" />
            </div>

            <div>
                <div className="text-[11px] font-medium text-slate-800 mb-2">Bureau Scores</div>
...
function CreditAnalysisCard() {
    return (
        <motion.div
            ...
            aria-live="polite"
        >
            <div className="text-[10px] font-medium mb-3">View Credit Analysis</div>

            <div className="text-[12px]">
                <div className="mb-2">
                    <span className="font-semibold">Payment History:</span>{" "}
                    <span className="text-muted-foreground">
                        Excellent track record with 98% on-time payments over the past 24 months. Only 2 late payments, both less than 30 days overdue.
                    </span>
                </div>
                ...
                <div className="mb-2">
                    ...
                    <span className="text-muted-foreground">
                        Currently at 65% of available credit limits. While this is higher than ideal, it reflects the company's growth phase and working capital needs.
                    </span>
                </div>
                ...
                <div className="mb-2">
                    ...
                    <span className="text-muted-foreground">
                        Low probability of default (2.5%) based on strong cash flows, diversified revenue streams, and consistent profitability growth.
                    </span>
                </div>
            </div>
        </motion.div>
...
function RiskFactorsCard() {
    ...
    return (
        <motion.div
            ...
            aria-live="polite"
        >
            <div className="text-[10px] font-medium mb-3">Risk Factors</div>
