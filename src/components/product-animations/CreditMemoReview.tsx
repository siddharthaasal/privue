...
export function Frame1Upload({ stage, uploadPct }: { stage: Stage; uploadPct: number }) {
  ...
  const files = [{ id: 'f1', name: 'Corporate Credit Proposal.pdf', size: '1.04 MB' }];
...
export function Frame3MemoInfo({
  ...
  return (
    <motion.div
      ...
      className="w-[300px] origin-bottom-right scale-[0.8] rounded-lg border bg-white p-3 shadow-sm backdrop-blur-sm sm:scale-[0.9] md:w-[300px] md:scale-100"
    >
      {/* TITLE */}
      <h3 className="mb-2 text-[11px] font-medium text-slate-800">Credit Memo Information</h3>
      ...
      <div className="mb-3">
        ...
        <div className="bg-privue-100/80 text-privue-700 mt-1 inline-block rounded-md px-2 py-[1.5px] text-[10px]">
          {memoId}
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <div className="max-w-[160px] truncate text-[11px] text-slate-800">{fileName}</div>
            <div className="text-[9px] text-slate-500">Credit Memo ({fileSize})</div>
          </div>
        </div>

        <button
          onClick={onOpenPdf}
          className="text-privue-700 text-[10px] whitespace-nowrap hover:underline"
        >
          View PDF
        </button>
      </motion.div>
...
export function Frame4Regulations({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -4 }}
      transition={{ duration: 0.25 }}
      className="w-[300px] origin-bottom-right scale-[0.75] rounded-xl bg-white p-4 shadow-sm backdrop-blur-sm sm:scale-[0.9] md:w-[330px] md:scale-100"
    >
      {/* TITLE */}
        ...
        {regs.map((r, i) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <div className="text-[11px] font-medium text-slate-900">{r}</div>
                <div className="text-[9px] text-slate-500">Preloaded</div>
              </div>
            </div>
      ...
      <motion.button
        ...
        onClick={onVerify}
        disabled={verifying}
        className="bg-privue-600 w-full rounded-md py-1.5 text-[10px] font-medium text-white shadow-sm"
      >
...
export function Frame5Results({
  ...
  return (
    <motion.div
      ...
      className="max-h-[280px] w-[350px] origin-bottom-right scale-[0.75] overflow-y-auto rounded-lg bg-white/95 p-4 shadow-sm backdrop-blur-sm sm:scale-[0.9] md:max-h-[280px] md:w-[500px] md:max-w-[540px] md:scale-100 md:p-3"
    >
      <div className="mb-1 text-[10px] font-medium text-slate-700">Verification results</div>

      {/* TABLE */}
      <div className="overflow-hidden rounded-md border bg-white text-[9px] text-black">
        {/* HEADER */}
        <div className="grid grid-cols-4 border-b bg-slate-50 md:grid-cols-4">
          <div className="px-1.5 py-1 text-[10px] font-medium">Page</div>
          <div className="px-1.5 py-1 text-[10px] font-medium">Category</div>
          <div className="px-1.5 py-1 text-[10px] font-medium">Issue Type</div>
          <div className="px-1.5 py-1 text-right text-[10px] font-medium">Status</div>
        </div>
        ...
        {rows.map((r, i) => (
          <motion.li
            ...
            className="grid grid-cols-4 items-center border-b px-1.5 py-1 md:grid-cols-4"
          >
            <div className="truncate">{r.page}</div>
            <div className="truncate">{r.category}</div>
            <div className="hidden truncate sm:block">{r.issueType}</div>
            <div
...
export default function CreditMemoReview() {
  ...
  const labels = [
    'Analyzing document',
    'Extracting credit info',
    'Preloading Regulations, Guidelines and Policy ',
  ];
