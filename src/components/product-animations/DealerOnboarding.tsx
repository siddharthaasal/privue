...
function DummyFormCompactSlow({
  ...
  return (
    ...
    >
      ...
      <input
        value={maskLastNDigits(mobile, 5)}
        readOnly
        className="w-full rounded-md border border-slate-200 p-1 font-sans text-xs tracking-wide"
      />
...
function DealerRow({ d, highlightId }: { d: Dealer; index: number; highlightId: string | null }) {
  ...
  return (
    ...
    >
      ...
      <td className="px-2 md:px-3 py-2 md:py-3 text-[10px] md:text-[12px]">{d.state}</td>
        ...
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="px-2 md:px-3 py-2 md:py-3 text-left text-[9px] md:text-[11px]">State</th>
