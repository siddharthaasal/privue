  ...
  return (
    ...
    >
      ...
      <div className="mb-3">
        ...
        <motion.div
          className="relative mt-1 w-full rounded-md border bg-white px-2 py-1.5 text-[11px]"
          ...
          <AnimatePresence>
            {step === 1 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -4 }}
                className="absolute top-[100%] left-0 z-10 mt-1 w-full rounded-md border bg-white shadow-md"
              >
                {customers.map((c, i) => (
                  <motion.div
                    key={i}
                    className={`cursor-pointer px-2 py-1.5 text-[11px] ${
                      i === 1 ? 'bg-privue-100 text-privue-700' : 'hover:bg-gray-50'
                    }`}
                  >
                    {c.name}
                  </motion.div>
      ...
      <div className="mb-3">
        ...
        <div className="mt-1 inline-block rounded-md bg-slate-100 px-2 py-[1.5px] text-[10px] text-slate-700">
          {selectedCustomer?.id || '---'}
        </div>
      </div>
              ...
              >
                {selectedCustomer.facilities.map((f: string, i: number) => (
                  <motion.div
                    key={i}
                    className={`cursor-pointer px-2 py-1.5 text-[11px] ${
                      i === 0 ? 'bg-privue-100 text-privue-700' : 'hover:bg-gray-50'
                    }`}
                  >
                    {f}
                  </motion.div>
...
export function Frame3API({
  ...
  return (
    ...
    >
      {/* TITLE */}
      <div className="mb-3 text-[10px] font-medium text-slate-700">APIs</div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <div className="text-[9px] text-slate-500">Fetched</div>
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="overflow-hidden rounded-md border bg-white text-[9px] text-black">
        {/* HEADER */}
        <div className="grid grid-cols-3 border-b bg-slate-50 md:grid-cols-3">
          <div className="px-1.5 py-1 text-[10px] font-medium">Category</div>
          <div className="px-1.5 py-1 text-[10px] font-medium">Indicator</div>
          {/* <div className="px-1.5 py-1 font-medium text-[10px]">Tag</div> */}
          <div className="px-1.5 py-1 text-right text-[10px] font-medium">Level</div>
        </div>
        ...
        {rows.map((r, i) => (
          <motion.li
            ...
            className="grid grid-cols-3 items-center border-b px-1.5 py-1 md:grid-cols-3"
          >
...
export default function EarlyWarningIndicatorsDemo() {
  ...
  const [_statuses, setStatuses] = useState<Status[]>(['pending', 'pending', 'pending', 'pending']);
  const labels = [
    'Analyzing document',
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-lg bg-white">
      ...
      <div className="absolute right-0.5 bottom-0.5 rounded-md shadow-xs md:right-6 md:bottom-6 md:shadow-xl md:backdrop-blur-3xl">
        ...
        {carouselStep === 'frame2' && <Frame2Upload key="f2" stage={stage} uploadPct={uploadPct} />}
