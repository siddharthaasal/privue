...
export default function FormToVerifiedFlow({
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="relative rounded-md overflow-hidden bg-white shadow-sm border">
                {/* base form image */}
                    ...
                    <div className="absolute right-4 top-4 w-64">
                        ...
                        <AnimatePresence mode="wait">
                            {step === "form" && (
                                ...
                                >
                                    <div className="bg-white/95 p-3 rounded-md shadow flex flex-col gap-2">
                                        <div className="text-sm font-medium">Review details</div>
                                        <div className="text-xs text-slate-500">Make sure GSTIN & mobile are correct</div>
                            ...
                            {step === "added" && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                >
                                    <div className="bg-white/95 p-3 rounded-md shadow">
                            ...
                            {step === "upload" && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                >
                                    <div className="bg-white/95 p-3 rounded-md shadow border">
                                        <div className="text-sm font-medium">Uploading documents</div>
                                        <div className="text-xs text-slate-500 mb-2">proof_of_dealer.pdf</div>
                                        ...
                                        <div className="text-xs text-slate-400 mt-2">{uploadProgress}%</div>
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {step === "done" && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                >
                                    <div className="bg-white/95 p-3 rounded-md shadow flex items-center gap-3">
                                        <div className="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center text-green-800">✓</div>
                                        <div>
                                            <div className="text-sm font-semibold">Verified</div>
                                            <div className="text-xs text-slate-500">Dealer status updated</div>
                                        </div>
                                        <div className="ml-auto">
                                            <button onClick={restart} className="text-xs underline text-slate-600">Run again</button>
                                        </div>
                                    </div>
                                </motion.div>
                            )}
                ...
                <div className="absolute left-4 bottom-4">
                    <div className="bg-white/90 px-3 py-1 rounded-md shadow text-sm">
...
function SmallLoaderLabel({ title, subtitle }: { title: string; subtitle?: string }) {
    return (
        <div className="bg-white/95 p-3 rounded-md shadow flex gap-3 items-center">
            <svg className="animate-spin" width="28" height="28" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="12" r="10" stroke="#e6e6e6" strokeWidth="3"></circle>
                <path d="M22 12a10 10 0 00-10-10" stroke="#111827" strokeWidth="3" strokeLinecap="round"></path>
            </svg>
            <div>
