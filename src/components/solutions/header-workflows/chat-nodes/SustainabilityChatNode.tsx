...
export function SustainabilityChatNodeInner(): any {
    ...
    const TypingIndicator: React.FC = () => (
      ...
      `}</style>
            ...
            >
                <span
                    className="inline-block w-[6px] h-[6px] rounded-full mx-[5px]"
        ...
        return (
            <div className="w-full">
                <div className="rounded-md border" style={{ borderColor: "rgba(15,23,36,0.06)" }}>
                    <div className="grid grid-cols-4 gap-1 px-2 py-1 text-[11px] text-slate-500 font-medium border-b" style={{ borderColor: "rgba(15,23,36,0.04)" }}>
                        <div>Location</div>
                    ...
                    <div className="flex flex-col">
                        {data.map((row, idx) => {
                            ...
                            return (
                                <div
                                    key={row.location}
                                    className="grid grid-cols-4 gap-1 px-1 py-1 items-center text-[10px] bg-white"
                                    style={{
                                ...
                                >
                                    ...
                                    <div>
                                        <span
                                            style={{
                                                ...
                                                borderRadius: 999,
                                                fontSize: 11,
                                                background: row.riskScore === "High" ? "rgba(239,68,68,0.08)" : row.riskScore === "Moderate" ? "rgba(245,158,11,0.08)" : "rgba(16,185,129,0.06)",
                                                color: row.riskScore === "High" ? "#ef4444" : row.riskScore === "Moderate" ? "#b45309" : "#065f46",
                                    ...
                                    <div className="pr-2">
                                        <div className="text-[11px] text-slate-600 mb-1">{row.vulnerability.toFixed(1)}/10</div>
                                    </div>
                                </div>
                            );
                        })}
    ...
    return (
        ...
        >
            ...
            <div className="absolute left-0 right-0 top-[18%] bottom-16 overflow-hidden pointer-events-none flex flex-col justify-end">
                <div className="relative w-full pointer-events-auto flex flex-col gap-3 px-3">
                    ...
                    {(phase === 2 || phase === 3 || phase === 4) && (
                        <article className="self-end rounded-[14px] bg-white shadow-sm p-2 border" style={{ borderColor: "rgba(15,23,36,0.04)" }}>
                            <div className="pl-3 pr-2 flex items-end gap-2">
                                <p className="m-0 text-[13px] text-[#0f1724] leading-[1.42] text-right flex-1">
                                    {qIndex === 0 ? "What are the potential climate risks for the facility?" : "Can you estimate the carbon emission of these suppliers?"}
                                </p>
                            </div>
                    ...
                    {phase === 4 && (
                        <article className="self-start bg-white border rounded-[14px] p-3" style={{ borderColor: "rgba(15,23,36,0.04)" }}>
                            ...
                            {qIndex === 1 && (
                                <div className="flex flex-col gap-3">
                                    <div className="text-[12px] text-slate-600">Estimated relative emissions (MtCO₂e share approximation).</div>
                                    <div className="rounded-md bg-[rgba(15,23,36,0.02)] p-2">
                                        <MinimalEmissionsChart data={SUPPLIER_EMISSIONS} />
                                    </div>
                                    <div className="text-[11px] text-slate-500 italic">Values are illustrative and shown as a relative comparison.</div>
                                </div>
                            )}
            ...
            <Handle
                ...
                id="chat-right"
                style={{ right: -8, width: 8, height: 8, borderRadius: 999, background: "var(--privue-700,#475569)", border: "none", top: "45%" }}
            />
        </div>
    );
}
