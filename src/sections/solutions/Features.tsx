        ...
        <div className="mx-auto mt-8 grid max-w-sm gap-6 *:text-center md:mt-16 @min-4xl:max-w-full @min-4xl:grid-cols-3">
          ...
          {solutions.map((s) => {
            ...
            const description =
              s.subHeading ||
              s.mainSolnDesc ||
              (s.featurePoints && s.featurePoints.length
                ? s.featurePoints.slice(0, 3).join(' · ')
                : '');
            ...
            return (
              <Card key={s.id} className="group shadow-zinc-950/5">
                <CardHeader className="pb-0">
                  ...
                  <h3 className="mt-6 text-left font-medium">{title}</h3>
                </CardHeader>

                <CardContent>
                  <p className="line-clamp-3 text-left text-sm">{description}</p>
...
const CardDecorator = ({ children }: { children: React.ReactNode }) => (
  <div className="relative mx-auto size-36 duration-200 [--color-border:color-mix(in_oklab,var(--color-zinc-950)10%,transparent)] group-hover:[--color-border:color-mix(in_oklab,var(--color-zinc-950)20%,transparent)] dark:[--color-border:color-mix(in_oklab,var(--color-white)15%,transparent)] dark:group-hover:bg-white/5 dark:group-hover:[--color-border:color-mix(in_oklab,var(--color-white)20%,transparent)]">
    ...
    />
    <div aria-hidden className="to-background absolute inset-0 bg-radial from-transparent to-65%" />
